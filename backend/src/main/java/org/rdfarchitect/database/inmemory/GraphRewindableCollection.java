...
/**
 * This class is a collection of {@link GraphRewindableWithUUIDs GraphRewindables} it also contains a universal {@link PrefixMapping}.
 ...
 */
@NoArgsConstructor
public class GraphRewindableCollection {
    ...
    //holds graphs
    private final ConcurrentMap<String, GraphRewindableWithUUIDs> graphs = new ConcurrentHashMap<>();
    ...
    public GraphRewindableCollection(Dataset dataset) {
        lock.lock();
        try {
            this.prefixes.setNsPrefixes(dataset.getPrefixMapping());
            if (!dataset.getDefaultModel().isEmpty()) {
                var graph = new GraphRewindableWithUUIDs(dataset.getDefaultModel().getGraph(), maxVersions, compressCount);
                graphs.put(DEFAULT_GRAPH_NAME, graph);
            }
            for (Iterator<Resource> it = dataset.listModelNames(); it.hasNext(); ) {
                var graphURI = it.next().getURI();
                var graph = new GraphRewindableWithUUIDs(dataset.getNamedModel(graphURI).getGraph(), maxVersions, compressCount);
    ...
    /**
     * Get a {@link GraphRewindableWithUUIDs} from the collection.
     *
     * @param graphUri The GraphUri to identify the Graph.
     *
     * @return The {@link GraphRewindableWithUUIDs}.
     */
    public GraphRewindableWithUUIDs getGraph(String graphUri) {
        lock.lock();
        try {
            graphUri = prefixes.expandPrefix(graphUri);
            createGraphIfNonExistent(graphUri);
    ...
    private void createGraphIfNonExistent(String graphUri) {
        ...
        if (!graphs.containsKey(graphUri)) {
            ...
            graphs.put(DEFAULT_GRAPH_NAME, new GraphRewindableWithUUIDs(GraphFactory.createDefaultGraph(), maxVersions, compressCount));
        }
    }
    ...
    public void create(String graphUri, Graph newGraph) {
        ...
        try {
            graphUri = prefixes.expandPrefix(graphUri);
            assertValidGraphName(graphUri);
            var graph = new GraphRewindableWithUUIDs(newGraph, maxVersions, compressCount);
            graphs.put(graphUri, graph);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Checks Whether {@link GraphRewindableCollection this} contains a certain {@link GraphRewindableWithUUIDs}
     *
     * @param graphUri The GraphUri to identify the Graph.
     *
     * @return True if {@link GraphRewindableCollection this} contains the {@link GraphRewindableWithUUIDs}, otherwise false
     *
     * @throws IllegalArgumentException if the graphUri is not a valid URI.
     */
    ...
    /**
     * Lists the {@link PrefixMapping} of {@link GraphRewindableCollection this}
     *
    ...
    /**
     * Replace the entire {@link PrefixMapping} of {@link GraphRewindableCollection this}. The internal {@link PrefixMapping} does not reference the given one to prevent
    ...
    public boolean canUndo(String graphUri) {
        ...
        try {
            ...
            assertValidGraphName(graphUri);
            assertThatGraphExists(graphUri);
            return graphs.get(graphUri).canUndo();
        } finally {
            lock.unlock();
        }
    }
    ...
    public boolean canRedo(String graphUri) {
        ...
        try {
            ...
            assertValidGraphName(graphUri);
            assertThatGraphExists(graphUri);
            return graphs.get(graphUri).canRedo();
        } finally {
            lock.unlock();
        }
    }
    ...
    public void restore(String graphUri, UUID versionId) {
        ...
        try {
            ...
            assertValidGraphName(graphUri);
            assertThatGraphExists(graphUri);
            graphs.get(graphUri).restore(versionId);
        } finally {
            lock.unlock();
        }
    }
