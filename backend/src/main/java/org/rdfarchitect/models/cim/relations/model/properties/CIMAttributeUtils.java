...
public class CIMAttributeUtils {
    ...
    public boolean hasPrimitiveDatatype(Resource attribute) {
        ...
        return datatype.hasProperty(CIMS.stereotype, attribute.getModel().createLiteral(CIMStereotypes.primitiveString));
    }
    ...
    public boolean hasCIMDatatype(Resource attribute) {
        ...
        var datatype = attribute.getProperty(CIMS.datatype).getResource();
        return datatype.hasProperty(CIMS.stereotype, attribute.getModel().createLiteral(CIMStereotypes.cimDatatypeString));
    }
    ...
    public boolean hasEnumAttribute(Resource attribute) {
        if (!CIMPropertyUtils.isAttribute(attribute) || !(attribute.hasProperty(RDFS.range) || attribute.hasProperty(CIMS.datatype))) {
            return false;
        }
    ...
    /**
     * fetches the primitive datatype of an attribute, Allows for primitive mappable datatypes and CIMDatatypes containing a value attribute.
     *
     * @param attribute the attribute
     *
    ...
    public RDFDatatype getPrimitiveDatatype(Resource attribute) {
        ...
        if (hasPrimitiveDatatype(attribute)) {
            var datatype = attribute.getProperty(ontology.createProperty(CIMS.datatype.getURI())).getResource();
        ...
        if (hasCIMDatatype(attribute)) {
            ...
            try (var quexec = QueryExecutionFactory.create(query, ontology)) {
                ...
                if (resultSet.hasNext()) {
                    var resultLabel = resultSet.next()
    ...
    public Set<Resource> listEnumDatatypeEntries(Resource attribute) {
        if (!CIMPropertyUtils.isAttribute(attribute) || !hasEnumAttribute(attribute)) {
            throw new IllegalArgumentException("Property is not an attribute or does not have a datatype");
        }
        var ontology = attribute.getModel();
